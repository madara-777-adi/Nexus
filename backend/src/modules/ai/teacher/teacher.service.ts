...
import ConceptModel, {
  ITopic,
  ILessonNode,
} from "../../concept/models/concept.model";
...
export class TeacherService {
  ...
  async generateTier2Subtopics(context: {
    ...
    if (!concept) {
      throw new NotFoundError(
        `Concept/Unit not found for ID: ${context.conceptId}`,
      );
    }
    ...
    if (!concept.owner.equals(ownerObjectId)) {
      throw new ForbiddenError(
        "You do not have permission to access this concept.",
      );
    }
  ...
  async generateLessonExperience(context: {
    ...
    return this.executeWithTransactionFallback(async (session) => {
      ...
      const mappedQuizQuestions = quizData.map((q: any) => ({
        question: q?.question || "Question",
        options: Array.isArray(q?.options) ? q.options : [],
        answerIndex: typeof q?.answerIndex === "number" ? q.answerIndex : 0,
      }));

      if (mappedQuizQuestions.length === 0) {
        throw new InternalServerError(
          "AI generated invalid quiz data. Please try again.",
        );
      }
      for (const q of mappedQuizQuestions) {
